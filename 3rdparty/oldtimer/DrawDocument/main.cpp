...
static const int DefaultFonzSize = 22;
  ...
  : QAbstractScrollArea(0),lineTimer(0),scaleFaktor(1.111),_doc(new QTextDocument("Hello World")),portrait_mode(true)
{
  ...
  resize(900,499);
      ...
      QAction *act_file_open = new QAction(tr("Open File html"), this);
      act_file_open->setShortcut(QKeySequence(QKeySequence::Open));
      connect(act_file_open, SIGNAL(triggered()),this, SLOT(OpenFile()));
      
  
        QIcon nowidactive =  createColorIcon(QColor("red"));
        QSystemTrayIcon *trayx = new QSystemTrayIcon(nowidactive);
        ...
        trayx->setVisible(true);
        trayx->setContextMenu(menu);
        trayx->showMessage(QString("Loading ox..."),QString("Lorem ipsum is here..."),nowidactive,15000);
...
{
   QApplication a(argc, argv);   //// qApp->applicationDirPath()
   EditArea w;
   w.show();
   return a.exec();
}
