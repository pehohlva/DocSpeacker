...
bool DrawDocument::gestureNative(QNativeGestureEvent * e) {
   ...
   //// xcout << "----------" << (int)e->type() << "---------------swip (" << e->value() << ")" << dir << "x" << POINT_TO_MM(distanceswip) << "\r";
   e->accept();
   return true;
}
bool DrawDocument::event(QEvent * e) {
   ...
   /// QTextStream xcout(stdout, QIODevice::WriteOnly);
  /// xcout << "E," << strak << " - " << __FUNCTION__ << " ev." << xmac << foo  << "\r";
...
void DrawDocument::adjustScrollbars() {
   ...
   mcurrent = QTransform(scaleFaktor, 0., 0., scaleFaktor, border_wi + PAPERSPACE, SLIDERMARGIN_TICK_TOTAL + PAPERSPACE);
...
void DrawDocument::paintEditPage(const int index, QPainter * painter) {
   ...
   QPointF center =  view.center();
    QTextStream xcout(stdout, QIODevice::WriteOnly);
    ////xcout << "------------Paint Page:" << index << " center ->" << center.x() << " \r";
}
...
void DrawDocument::fitToNormal() {
   setZoom(1.7);
}

void DrawDocument::resizeEvent(QResizeEvent * e) {
   this->updateslow();
}
...
void DrawDocument::ShowFile(const QString file) {
  /// test if a pdf!
   ...
   if ( RUNPDF ) {
       RUNPDF = false;
       m_pdf->loadFile("../notexist.pdf"); /// make not valid..to clas not repaint..
     }
#endif
   int casego = 0;
   ...
   bool ok = false;
    ...
    if (flat.indexOf(".html",0) > 0) {
       ...
       if (buf->isValid()) {
        ...
        casego = 2;
       doc->setHtml(buf->tohtmlsimulate());
    } else if ( (flat.indexOf(".pdf",0) > 0 ) || (flat.indexOf("acrobat",0) > 0 )  ) {
       casego = 3;
#ifdef LOADPDFIUMYES
...
void DrawDocument::setZoom(const qreal value) {
   if (value < 0.22 || value > 10) {
      ...
      scaleFaktor = value;
      QTimer::singleShot(35, this, SLOT(updateslow()));
   }
}
...
QImage DrawDocument::req_ImagePdf(const int  id, QSize *size, const QSize &requestedSize)
{
#ifdef LOADPDFIUMYES
        ...
        qreal scale = requestedSize.isValid() ? requestedSize.width()/(qreal)page.width() :2;
...
/*
bool DrawDocument::gestureEvent(QGestureEvent * e) {
