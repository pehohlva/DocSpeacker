...
bool QUnitMargin::event ( QEvent * e ) 
{
    ...
    if (e->type() == QEvent::MouseMove ||
        ...
        HandleMove(ke0->localPos());
        e->setAccepted(true);
    } else if (e->type() == QEvent::MouseButtonDblClick ) {
        QMouseEvent *ke = static_cast<QMouseEvent *>(e);
        
        if (ke->localPos().x() < (width() / 2)) {
            Cursor_1_X = qBound(1.0,ke->localPos().x(),Cursor_2_X - MaximumCollisionAllowed - 1.0);
        } else {
            Cursor_2_X = qBound(Cursor_1_X + MaximumCollisionAllowed + 1.0,ke->localPos().x(),width()- 1.0);
        }
        HandleMove(ke->localPos());
        e->setAccepted(true);
    }
